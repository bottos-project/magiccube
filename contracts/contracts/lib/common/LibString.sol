...
library LibString {
    ...
    function getUintValueByKey(string _self, string _key) internal returns (uint _ret) {
        _ret = (uint)(getIntValueByKey(_self, _key));
    }
    ...
    function isStringNull(string _self) internal returns (bool) {
        bytes memory tmpString = bytes(_self);
