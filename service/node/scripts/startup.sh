...
function eoscheck()
{
	...
	then
		echo -e "\033[33m *WRAN* set EOS runtime environment , it may take a moment ... \033[0m"
	    eosdepends
	fi
}
...
function starteos()
{
    if [ ! -e ${GO_PATH}/bin/eosd ];
	then
		echo -e "\033[31m *ERROR* Fail to find eosd process under :"${GO_PATH}"/bin !!! \033[0m"
		exit 1
	fi
	
	if [ ! -d ${GO_PATH}/bin/data-dir ] || [ ! -f ${GO_PATH}/bin/data-dir/config.ini ] || [ ! -f ${GO_PATH}/bin/data-dir/genesis.json ];
	then
		echo -e "\033[31m *ERROR* Fail to find configuration item : data-dir or data-dir/config.ini under :"${GO_PATH}"/bin \033[0m"
		exit 1
	fi
	...
	CHK_EOS=`ps -elf | grep eosd | grep -v grep | wc -l`
	if [ "$CHK_EOS" -lt 1 ];
	then 
		#start eosd process  , nohup "command" > myout.file 2>&1 &
		nohup ${GO_PATH}/bin/eosd --http-server-address ${SERVER_IPADR}:${CHAIN_PORT} -m mongodb://127.0.0.1/bottos --resync > eosd.file 2>&1 &
		sleep 3
	fi
    return
}
...
function stopeos()
{
    RUNNING_EOS_PID=`ps -elf | grep eosd | grep -v grep | awk '{print $4}'`
    if [ -z "$RUNNING_EOS_PID" ];
    then
        echo -e "\033[33m *WRAN* EOS process hadn't been running ... \033[0m"
        exit 1
    fi

    kill -SIGINT $RUNNING_EOS_PID
...
function startminio()
{
	...
	#${SERVER_PATH}minio server "$MINIO_OPTS" "$MINIO_VOLUMES" > minio.file 2>&1 &
...
function registnode()
{
    if [ ! -e ${GO_PATH}/bin/eosc ];
	then
	    echo -e "\033[31m *ERROR* Fail to find eosc process under :"${GO_PATH}"/bin !!! \033[0m"
		exit 1
	fi

    NODE_UUID=`cat /sys/devices/virtual/dmi/id/product_uuid`
    UNIQUE_IDENT=${NODE_UUID}"-"${SERVER_IPADR}
    ...
    #get city info by ip address
    CITY_NAME=`curl http://ip.taobao.com/service/getIpInfo.php?ip=47.98.47.148 | jq '.data.city' | tr -d '"'`

    #get longitude and latitude by city
    POINTX_INFO=`curl "http://apis.map.qq.com/jsapi?qt=poi&wd="${CITY_NAME}"&pn=0&rn=10&rich_source=qipao&rich=web&nj=0&c=1&key=FBOBZ-VODWU-C7SVF-B2BDI-UK3JE-YBFUS&output=jsonp&pf=jsapi&ref=jsapi&cb=qq.maps._svcb3.search_service_0" | sed 's/.$//' | sed '1d' | sed '1i\{' | grep "pointx" | sed s/[[:space:]]//g | sed -n 1p | tr -d '"'`
    POINTY_INFO=`curl "http://apis.map.qq.com/jsapi?qt=poi&wd="${CITY_NAME}"&pn=0&rn=10&rich_source=qipao&rich=web&nj=0&c=1&key=FBOBZ-VODWU-C7SVF-B2BDI-UK3JE-YBFUS&output=jsonp&pf=jsapi&ref=jsapi&cb=qq.maps._svcb3.search_service_0" | sed 's/.$//' | sed '1d' | sed '1i\{' | grep "pointy" | sed s/[[:space:]]//g | sed -n 1p | tr -d '"'`

    #remove flied from string
    POINTX_INFO=${POINTX_INFO#*:}
    POINTY_INFO=${POINTY_INFO#*:}

    #join a string , for example , "{city}|{longitude}|{latitude}"
    NODE_INFO=${CITY_NAME}"|"${POINTX_INFO}"|"${POINTY_INFO}

    #register the string("{city}|{longitude}|{latitude}") into blockchain
    NODE_REG_RES=`${GO_PATH}/bin/eosc -H ${SERVER_IPADR} -p ${CHAIN_PORT} push message -s nodemng nodeinforeg "{\"node_id\":\"${UNIQUE_IDENT}\", \"basic_info\":{\"node_ip\":\"${SERVER_IPADR}\",\"node_port\":\"8080\",\"node_address\":\"${NODE_INFO}\"}}" -S nodemng`
    if [ ! -z `echo $NODE_REG_RES | grep Failed` ];
    then
        echo -e "\033[31m *ERROR* Failed to regist the node !!! \033[0m"
        return
    fi
...
case $1 in
    ...
    "restarteos")
        ...
        ;;
    "help"|*)
        echo -e "\033[32m you have to input a parameter , Please run the script like ./startup.sh deploy|start|stop|starteos|stopeos|restarteos !!! \033[0m"
        ;;
esac
