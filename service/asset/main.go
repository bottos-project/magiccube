...
func (u *Asset) QueryUploadedData(ctx context.Context, req *proto.QueryUploadedDataRequest, rsp *proto.QueryUploadedDataResponse) error {
	...
	mgo.DB("bottos").C("Messages").Find(where).Skip(skip).Limit(int(req.PageSize)).All(&ret)
	//mgo.DB("bottos").C("Messages").Find(where).Sort("data.basic_info.publish_date").Skip(skip).Limit(int(req.PageSize)).All(&ret)
...
func (u *Asset) Query(ctx context.Context, req *proto.QueryRequest, rsp *proto.QueryResponse) error {
	...
	mgo.DB("bottos").C("Messages").Find(where).Sort("-createdAt").Skip(skip).Limit(int(req.PageSize)).All(&ret)
	//mgo.DB("bottos").C("Messages").Find(where).Sort("data.basic_info.publish_date").Skip(skip).Limit(int(req.PageSize)).All(&ret)
	log.Info("ret:", ret)
	...
	for _, v := range ret {
		...
		//theTime, _ := time.ParseInLocation(timeLayout, v.CreatedAt, loc) //使用模板在对应时区转化为time.time类型

		//sr := theTime.Unix()
		log.Info("time:", v.CreatedAt)
...
func (u *Asset) QueryByID(ctx context.Context, req *proto.QueryByIDRequest, rsp *proto.QueryResponse) error {
	...
	rows, err := GetAssetById(req.AssetID)
	if err != nil {
		fmt.Println(err)
...
func GetAssetById(assertId string) ([]*proto.QueryRow, error) {
	var where interface{}
	where = &bson.M{"type": "assetreg"}
	log.Info(assertId)
	if assertId != "" {
	...
	//	log.Error(err)
	//}
	mgo.DB("bottos").C("Messages").Find(where).Limit(10).All(&ret)
	//mgo.DB("bottos").C("Messages").Find(where).Sort("data.basic_info.publish_date").Skip(skip).Limit(int(req.PageSize)).All(&ret)
...
func (u *Asset) GetUserPurchaseAssetList(ctx context.Context, req *proto.GetUserPurchaseAssetListRequest, rsp *proto.QueryResponse) error {
	...
	var rows = []*proto.QueryRow{}
	for _, v := range ret {
		asset, err := GetAssetById(v.Data.BasicInfo.AssetID)
		if err != nil {
			fmt.Println(err)
