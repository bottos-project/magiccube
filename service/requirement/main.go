...
func (u *Requirement) Query(ctx context.Context, req *requirement_proto.QueryRequest, rsp *requirement_proto.QueryResponse) error {
	...
	mgo.DB("bottos").C("Messages").Find(where).Sort("data.basic_info.publish_date").Skip(skip).Limit(int(pageSize)).Distinct("ass",&ret)
	...
	for _, v := range ret {
		rows = append(rows, &requirement_proto.QueryRow{
			...
			PublishDate : v.Data.BasicInfo.PublishDate,
		})
	}
